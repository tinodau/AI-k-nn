...
trainingFile= 'cobatraining.csv' # cobatraining.csv
testingFile = 'cobatest.csv'
split       = 1 # BEST 65
...
def getResultTest (trainingnya, dataTestSet,  testAnswer, k) :
    ...
    for i in range(len(testFilenya)) :
        ...
        print(i, testFilenya[i], testAnswer[i])
        testAnswer.append(vote)

    with open(time+" k" + str(k) +".csv", "wb") as startTest:
        writer = csv.writer(startTest)
        for row in range(len(testFilenya)) :
            writer.writerow([testFilenya[row], " ==> ", testAnswer[row]])
        startTest.close()
